        ...
        for (int i = 0; i < num_processes; i++) {
            ...
            if (processes[i].remaining_time > 0) {
                ...
                if (processes[i].remaining_time > quantum) {
                    ...
                    // Terminate the process when it completes
                    kill(processes[i].pid, SIGKILL); // Send termination signal
                    printf("Process %d (PID: %d) has been terminated.\n", processes[i].id, processes[i].pid);
                }

                // After execution, stop the process by sending SIGSTOP if it's not terminated
                if (processes[i].remaining_time > 0) {
